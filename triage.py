...
async def run(ecosystem: str, package: str, old_version: str, new_version: str) -> None:
    ...
    # Compact paste-ready block — one section, no repeated analysis.
    e = _VERDICT_EMOJI.get(v, "")
    flags_md = " · ".join(f"`{f}`" for f in verdict.flags) if verdict.flags else "none"
    repo_url = "https://github.com/temporal-community/dependency-scout"
    temporal_url = "https://temporal.io"
    print("─" * 60)
    print(
        f"{e} **{v.upper()}** ({verdict.confidence:.0%}) — "
        f"{package} `{old_version}` → `{new_version}` ({ecosystem})"
    )
    print(f"Flags: {flags_md}")
    print(
        f"<sub>Generated by [dependency-scout]({repo_url}) · "
        f"[Run on every PR with Temporal]({temporal_url})</sub>"
    )
    print("─" * 60)
