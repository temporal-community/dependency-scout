...
def _pypi_provenance(
    kind: str = "GitHub", repo: str = "psf/requests", with_dsse: bool = False
) -> dict:
    attestations: list = []
    if with_dsse:
...
async def test_npm_attestation_success():
    respx.get(f"{NPM_ATT_BASE}/lodash@4.17.20").mock(
        return_value=httpx.Response(
            200, json=_npm_attestation_bundle("https://github.com/lodash/lodash")
        )
    )
...
async def test_npm_attestation_publisher_changed():
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.0.0").mock(
        return_value=httpx.Response(
            200, json=_npm_attestation_bundle("https://github.com/original/mypkg")
        )
    )
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.1.0").mock(
        return_value=httpx.Response(
            200, json=_npm_attestation_bundle("https://github.com/attacker/mypkg")
        )
    )
...
async def test_npm_attestation_no_provenance_predicate_in_bundle():
    ...
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.0.0").mock(
        return_value=httpx.Response(
            200,
            json={"attestations": [{"predicateType": "https://in-toto.io/Link/v1", "bundle": {}}]},
        )
    )
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.1.0").mock(return_value=httpx.Response(404))
...
async def test_pypi_attestation_populates_publisher_account_age(monkeypatch):
    ...
    respx.get("https://api.github.com/users/psf").mock(
        return_value=httpx.Response(
            200, json={"login": "psf", "created_at": "2010-03-15T00:00:00Z"}
        )
    )
...
async def test_npm_attestation_populates_slsa_chain_fields():
    ...
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.0.0").mock(
        return_value=httpx.Response(
            200, json=_npm_attestation_bundle("https://github.com/owner/mypkg")
        )
    )
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.1.0").mock(
        return_value=httpx.Response(
            200,
            json=_npm_attestation_bundle(
...
def _signals_with_attestation(**overrides):
    ...
    return PackageChecks(
        ...
        attestation=AttestationChecks(
            ...
            old_publisher_repo=overrides.pop("old_publisher_repo", None),
...
async def test_publisher_changed_false_when_only_commit_sha_differs():
    ...
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.0.0").mock(
        return_value=httpx.Response(
            200, json=_npm_attestation_bundle("https://github.com/org/mypkg", commit_sha=old_sha)
        )
    )
    respx.get(f"{NPM_ATT_BASE}/mypkg@1.1.0").mock(
        return_value=httpx.Response(
            200, json=_npm_attestation_bundle("https://github.com/org/mypkg", commit_sha=new_sha)
        )
    )
